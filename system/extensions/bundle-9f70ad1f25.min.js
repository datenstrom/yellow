...
/* clarity.js */
const doc=document.documentElement;const inline=":inline";const parentURL='{{ absURL "" }}';let showImagePosition="{{ .Site.Params.figurePositionShow }}";const showImagePositionLabel='{{ .Site.Params.figurePositionLabel }}';function isObj(obj){return(obj&&typeof obj==='object'&&obj!==null)?!0:!1}
function createEl(element='div'){return document.createElement(element)}
function elem(selector,parent=document){let elem=parent.querySelector(selector);return elem!=!1?elem:!1}
function elems(selector,parent=document){let elems=parent.querySelectorAll(selector);return elems.length?elems:!1}
function pushClass(el,targetClass){if(isObj(el)&&targetClass){elClass=el.classList;elClass.contains(targetClass)?!1:elClass.add(targetClass)}}
function hasClasses(el){if(isObj(el)){const classes=el.classList;return classes.length}}(function markInlineCodeTags(){const codeBlocks=elems('code');if(codeBlocks){codeBlocks.forEach(function(codeBlock){containsClass(codeBlock,'verbatim')?pushClass(codeBlock,'noClass'):!1;hasClasses(codeBlock)?false:pushClass(codeBlock,'noClass')})}})();function deleteClass(el,targetClass){if(isObj(el)&&targetClass){elClass=el.classList;elClass.contains(targetClass)?elClass.remove(targetClass):!1}}
...
function containsClass(el,targetClass){if(isObj(el)&&targetClass&&el!==document){return el.classList.contains(targetClass)?!0:!1}}
function elemAttribute(elem,attr,value=null){if(value){elem.setAttribute(attr,value)}else{value=elem.getAttribute(attr);return value?value:!1}}
...
return matching.includes(!0)}
return element.matches(selectors)}}
function copyToClipboard(str){let copy,selection,selected;copy=createEl('textarea');copy.value=str;copy.setAttribute('readonly','');copy.style.position='absolute';copy.style.left='-9999px';selection=document.getSelection();doc.appendChild(copy);selected=selection.rangeCount>0?selection.getRangeAt(0):!1;copy.select();document.execCommand('copy');doc.removeChild(copy);if(selected){selection.removeAllRanges();selection.addRange(selected)}}
const iconsPath='{{ default "icons/" .Site.Params.iconsDir }}';function loadSvg(file,parent,path=iconsPath){const link=`${parentURL}${path}${file}.svg`;fetch(link).then((response)=>{return response.text()}).then((data)=>{parent.innerHTML=data})}
...
return"unknown"}
function horizontalSwipe(element,func,direction){let touchstartX=0;let touchendX=0;let swipeDirection=null;function handleGesure(){return(touchendX+50<touchstartX)?'left':(touchendX<touchstartX+50)?'right':!1}
...
swipeDirection===direction?func():!1})}
function parseBoolean(string){let bool;string=string.trim().toLowerCase();switch(string){case 'true':return!0;case 'false':return!1;default:return undefined}};(function(){const bodyElement=elem('body');const platform=navigator.platform.toLowerCase();if(platform.includes("win")){pushClass(bodyElement,'windows')}})();(function toggleColorModes(){const light='lit';const dark='dim';const storageKey='colorMode';const key='--color-mode';const data='data-mode';const bank=window.localStorage;function currentMode(){let acceptableChars=light+dark;acceptableChars=[...acceptableChars];let mode=getComputedStyle(doc).getPropertyValue(key).replace(/\"/g,'').trim();mode=[...mode].filter(function(letter){return acceptableChars.includes(letter)});return mode.join('')}
...
function setUserColorMode(mode=!1){const isDarkMode=currentMode()==dark;const storedMode=bank.getItem(storageKey);if(storedMode){if(mode){changeMode(isDarkMode)}else{elemAttribute(doc,data,storedMode)}}else{if(mode===!0){changeMode(isDarkMode)}}}
setUserColorMode();doc.addEventListener('click',function(event){let target=event.target;let modeClass='color_choice';let animateClass='color_animate';let isModeToggle=containsClass(target,modeClass);if(isModeToggle){pushClass(target,animateClass);setUserColorMode(!0)}})})();(function navToggle(){doc.addEventListener('click',function(event){const target=event.target;const open='jsopen';const navCloseIconClass='.nav_close';const navClose=elem(navCloseIconClass);const isNavToggle=target.matches(navCloseIconClass)||target.closest(navCloseIconClass);const harmburgerIcon=navClose.firstElementChild.firstElementChild;if(isNavToggle){event.preventDefault();modifyClass(doc,open);modifyClass(harmburgerIcon,'isopen')}
if(!target.closest('.nav')&&elem(`.${open}`)){modifyClass(doc,open);let navIsOpen=containsClass(doc,open);!navIsOpen?modifyClass(harmburgerIcon,'isopen'):!1}
